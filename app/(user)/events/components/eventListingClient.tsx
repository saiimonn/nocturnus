...
interface EventListingClientProps {
  initialEvents: Event[]
  clubs: Club[]
}
...
export default function EventListingClient({ initialEvents, clubs }: EventListingClientProps) {
  const [dateRange, setDateRange] = useState({ startDate: "", endDate: "" })
  const [search, setSearch] = useState("")
  const [activeEventId, setActiveEventId] = useState<string | null>(null)

  const getVenueByClubId = (clubId: string) => {
  ...
  }, [initialEvents, search, dateRange, clubs])
        ...
        {filteredEvents.map((event) => (
          ...
          >
            <EventCard
              ...
              venue={getVenueByClubId(event.club_id)}
            />
          </div>
        ))}
